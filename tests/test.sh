...
printf ${green}"Starting Docker container: theqwan/docker-$distro-ansible."${neutral}"\n"
docker pull theqwan/docker-$distro-ansible:latest
docker run --detach --volume="$role_dir":/etc/ansible/roles/role_under_test:rw --name $container_id $opts theqwan/docker-$distro-ansible:latest $init
else
